...
# Prior to running this script, ensure that the GCE VM service account is Project IAM Admin and Pub/Sub Admin in the IAM & admin page
# Run from root user when installing using the Lab VM, type sudo -i 
# If using Cloud Shell, no need to switch to root, use the setup.sh instead
...
mkdir bin
PATH=$PATH:$HOME/bin/
cp $HOME/advanced-kubernetes-workshop/terraform/connect.sh /usr/bin/.
chmod +x /usr/bin/connect.sh
sed -i -e s/ONE/$ONE/g -e s/TWO/$TWO/g /usr/bin/connect.sh
cp $HOME/advanced-kubernetes-workshop/services/frontend/get-ingress.sh /usr/bin/.
chmod +x /usr/bin/get-ingress.sh
sed -i -e s/ONE/$ONE/g -e s/TWO/$TWO/g /usr/bin/get-ingress.sh
cp $HOME/advanced-kubernetes-workshop/terraform/fortio.sh /usr/bin/.
chmod +x /usr/bin/fortio.sh 
echo "********************************************************************************"
...
sudo ln -s $HOME/kubectx/kubectx /usr/bin/kubectx
sudo ln -s $HOME/kubectx/kubens /usr/bin/kubens
echo "********************************************************************************"
...
# Install Skaffold
echo "${bold}Installing skaffold...${normal}"
cd $HOME
...
cp /usr/local/bin/html2text /usr/bin/
echo "********************************************************************************"

# Install pv
sudo apt-get update &> /dev/null
sudo apt-get install pv &> /dev/null
cp /usr/bin/pv /usr/bin/
...
cp $HOME/istio-$ISTIO_VERSION/bin/istioctl /usr/bin/.
echo "********************************************************************************"
...
export PROJECT=$(gcloud info --format='value(config.project)')
cd $HOME/advanced-kubernetes-workshop/terraform/vm
sed -i -e s/PROJECTID/$PROJECT/g $HOME/advanced-kubernetes-workshop/terraform/vm/main.tf
terraform init
