...
resource "null_resource" "gke-one-cluster" {
   ...
   provisioner "local-exec" {
        command = "helm install ~/istio-${var.istio-ver}/install/kubernetes/helm/istio --name istio --namespace istio-system --set kiali.enabled=true --set tracing.enabled=true --set global.mtls.enabled=true --set grafana.enabled=true --set servicegraph.enabled=true"
      }
   provisioner "local-exec" {
