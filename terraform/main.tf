...
resource "null_resource" "gke-one-cluster" {
   ...
   provisioner "local-exec" {
        command = "helm template ~/istio-${var.istio-ver}/install/kubernetes/helm/istio --name istio --namespace istio-system --set kiali.enabled=true --set tracing.enabled=true --set global.mtls.enabled=true --set grafana.enabled=true --set servicegraph.enabled=true > $HOME/istio.yaml"
      }
   provisioner "local-exec" {
        command = "kubectl create namespace istio-system --context ${google_container_cluster.gke-one.name}"
      }
   provisioner "local-exec" {
        command = "kubectl apply -f $HOME/istio.yaml --context ${google_container_cluster.gke-one.name}"
      }
   provisioner "local-exec" {
...
resource "null_resource" "gke-two-cluster" {
   ...
   provisioner "local-exec" {
        command = "kubectl create namespace istio-system --context ${google_container_cluster.gke-two.name}"
      }
   provisioner "local-exec" {
        command = "kubectl apply -f $HOME/istio.yaml --context ${google_container_cluster.gke-two.name}"
      }
   provisioner "local-exec" {
...
resource "null_resource" "gke-spinnaker-cluster" {
   ...
   provisioner "local-exec" {
        command = "helm install -n spin stable/spinnaker -f ~/advanced-kubernetes-workshop/terraform/spinconfig.yaml --timeout=600 --version 1.6.1 --kube-context ${google_container_cluster.gke-spinnaker.name}"
      }
   provisioner "local-exec" {
        command = "kubectl create namespace istio-system --context ${google_container_cluster.gke-spinnaker.name}"
      }
   provisioner "local-exec" {
        command = "kubectl apply -f $HOME/istio.yaml --context ${google_container_cluster.gke-spinnaker.name}"
      }
   provisioner "local-exec" {
