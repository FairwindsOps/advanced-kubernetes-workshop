resource "null_resource" "gke-two-cluster" {
  ...
  provisioner "local-exec" {
    command = "helm upgrade --install istio ~/istio-${var.istio-ver}/install/kubernetes/helm/istio --namespace istio-system --values ~/istio-${var.istio-ver}/install/kubernetes/helm/istio/values-istio-demo-auth.yaml --kube-context ${google_container_cluster.gke-two.name}"
  }
