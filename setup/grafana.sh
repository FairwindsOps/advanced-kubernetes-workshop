...
export GRAFANA_ONE=$(kubectl get pods -o wide -n istio-system --context gke-$ONE | grep grafana | awk 'NR==1 {print $6}')
export GRAFANA_TWO=$(kubectl get pods -o wide -n istio-system --context gke-$TWO | grep grafana | awk 'NR==1 {print $6}')
export GRAFANA_SPINNAKER=$(kubectl get pods -o wide -n istio-system --context gke-spinnaker | grep grafana | awk 'NR==1 {print $6}')
